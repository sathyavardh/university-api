...
import org.sathya.dao.StudentDAO;
            ...
            switch (method.toUpperCase()) {
                case "GET":
                    if (idParam != null) {
                        ...
                        if (student != null)
                            respond(exchange, 200, "application/json", gson.toJson(student));
                        else
                            respond(exchange, 404, "application/json", "{\"error\":\"Student not found\"}");
                    } else {
                        List<Student> students = dao.getAll();
                        respond(exchange, 200, "application/json", gson.toJson(students));
                    }
                    break;

                case "POST":
                    String postBody = readRequestBody(exchange);
                    Student newStudent = gson.fromJson(postBody, Student.class);
                    dao.save(newStudent);
                ...
                case "PUT":
                    ...
                    Student updatedStudent = gson.fromJson(putBody, Student.class);
                    updatedStudent.setId(Integer.parseInt(idParam));
